...
import { EditorAction, EditorState, TextStyle, parsePageOrder } from "./editor-types";
...
type Props = {
  ...
  setMoveDraft: (fn: (prev: { x: number; y: number; width: number; height: number }) => { x: number; y: number; width: number; height: number }) => void;
  overlayDraft: { text: string; x: number; y: number; width: number; height: number };
  setOverlayDraft: (fn: (prev: { text: string; x: number; y: number; width: number; height: number }) => { text: string; x: number; y: number; width: number; height: number }) => void;
...
function CollapseButton({ onClick }: { onClick: () => void }) {
  return (
    <button className="rounded-full bg-slate-100 p-1.5 text-slate-700 hover:bg-slate-200" onClick={onClick} title="Collapse Inspector Panel">
...
function TextFormatToolbar({ textStyle, setTextStyle }: { textStyle: TextStyle; setTextStyle: Props["setTextStyle"] }) {
  return (
      ...
      {(["bold", "italic", "underline"] as const).map((key) => (
        <button
          ...
          className={`w-8 h-8 rounded-lg border text-sm text-slate-700 transition-colors ${
            textStyle[key] ? "bg-slate-200 border-slate-400" : "border-slate-200 hover:bg-slate-100"
...
export function InspectorPanel({
  state, dispatch, isOpen, onToggle,
  selectedTextBlock, selectedImageBlock, currentPageRotation,
  textDraft, setTextDraft, textStyle, setTextStyle,
  moveDraft, setMoveDraft, overlayDraft, setOverlayDraft,
  pageOrderDraft, setPageOrderDraft,
  imageInputRef, overlayImageInputRef, onReplaceImage, onOverlayImage, onQueueOperation,
}: Props) {
  return (
    <aside className="glass-panel rounded-[24px] p-4 shadow-panel flex flex-col h-full min-h-0">
      {!isOpen ? (
        /* Collapsed sliver */
        <div className="flex flex-col items-center justify-start h-full pt-2">
          ...
          <div className="mt-8 flex-1" style={{ writingMode: "vertical-rl", textOrientation: "mixed" }}>
            <span className="text-xs font-semibold uppercase tracking-widest text-slate-500 whitespace-nowrap">Inspector</span>
          </div>
        </div>
      ) : (
        <div className="flex flex-col h-full min-h-0">
          ...
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-2">
            <div>
              ...
              <h2 className="mt-1 text-lg font-semibold">Selection &amp; queue</h2>
            </div>
          ...
          <div className="shrink-0 mt-4 rounded-3xl bg-white/85 p-4 text-sm">
            ...
            {selectedTextBlock && (
              <div className="mt-3 rounded-2xl border border-teal-200 bg-teal-50 px-3 py-3 text-teal-900">
                ...
                <p className="mt-1 text-xs leading-5 line-clamp-3">{selectedTextBlock.text}</p>
              </div>
            )}
            {selectedImageBlock && (
          ...
          {/* Scrollable tools */}
          <div className="flex-1 mt-4 min-h-0 overflow-y-auto space-y-4 pr-1 pb-4">
            ...
            {selectedTextBlock && (
              <div className="rounded-3xl bg-white/85 p-4">
                ...
                <textarea
                  ...
                  value={textDraft}
                />
                <TextFormatToolbar textStyle={textStyle} setTextStyle={setTextStyle} />
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <button
                ...
                <p className="mt-3 text-xs text-slate-500">Position &amp; size (x, y, w, h)</p>
                <BoundsInputs
                  value={moveDraft}
                  onChange={(field, v) => setMoveDraft((d) => ({ ...d, [field]: v }))}
                />
              </div>
            )}
            ...
            {selectedImageBlock && (
              <div className="rounded-3xl bg-white/85 p-4">
                ...
                <input accept="image/*" className="hidden" onChange={onReplaceImage} ref={imageInputRef} type="file" />
              </div>
            )}
            ...
            <div className="rounded-3xl bg-white/85 p-4">
              ...
              <BoundsInputs
                value={overlayDraft}
                onChange={(field, v) => setOverlayDraft((d) => ({ ...d, [field]: v }))}
              />
              <div className="mt-3 flex flex-wrap gap-2">
                <button
              ...
              <input accept="image/*" className="hidden" onChange={onOverlayImage} ref={overlayImageInputRef} type="file" />
            </div>
            ...
            <div className="rounded-3xl bg-white/85 p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <button
                  ...
                  onClick={() => onQueueOperation({ type: "delete_page", page_number: state.selectedPage })}
                  type="button"
                >
                  Delete page
                </button>
              ...
              <button
                ...
                onClick={() => onQueueOperation({ type: "reorder_pages", page_order: parsePageOrder(pageOrderDraft) })}
                type="button"
              >
                Queue page reorder
              </button>
            ...
            <div className="rounded-3xl bg-slate-950 px-4 py-4 text-white">
              <div className="flex items-center justify-between">
                ...
                <span className="rounded-full bg-white/10 px-2.5 py-1 text-xs">{state.operations.length} queued</span>
              </div>
              <div className="mt-3 space-y-2">
                {state.operations.length > 0 ? (
                  state.operations.map((op, i) => (
                    <div className="rounded-2xl border border-white/10 bg-white/5 px-3 py-3 text-xs" key={`${op.type}-${i}`}>
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="font-semibold uppercase tracking-[0.24em] text-teal-200">{op.type.replaceAll("_", " ")}</p>
                          <p className="mt-1 break-all text-slate-300">{JSON.stringify(op)}</p>
                        </div>
                        <button
                          className="rounded-full border border-white/15 px-2 py-1 text-[11px] text-slate-200 hover:bg-white/10"
                          onClick={() => dispatch({ type: "remove_operation", payload: i })}
                          type="button"
                        >
                          Remove
                        </button>
                  ...
                  <p className="text-sm text-slate-300">Queue edits here, click Apply Changes to update the live preview, then export when ready.</p>
                )}
              </div>
            </div>
