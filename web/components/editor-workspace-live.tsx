...
import { applyDocument, deleteDocument, exportDocument, getManifest, listDocuments, toApiUrl, uploadDocument } from "@/lib/api";
...
import {
  ...
  combineWarnings, fileToDataUrl, TextStyle,
} from "./editor-types";
...
export function EditorWorkspaceLive() {
  ...
  const [textStyle, setTextStyle] = useState<TextStyle>({ fontSize: 12, bold: false, italic: false, underline: false, color: "#000000" });
  ...
  const currentPage = useMemo<PageManifest | null>(
    () => state.manifest?.pages.find((p) => p.page_number === state.selectedPage) ?? null,
    [state.manifest, state.selectedPage],
  );
  const selectedTextBlock = useMemo<TextBlock | null>(
    () => currentPage?.text_blocks.find((b) => b.id === state.selectedTextBlockId) ?? null,
    [currentPage, state.selectedTextBlockId],
  );
  const selectedImageBlock = useMemo<ImageBlock | null>(
    () => currentPage?.image_blocks.find((b) => b.id === state.selectedImageBlockId) ?? null,
  ...
  useEffect(() => {
    ...
    setTextStyle((s) => ({ ...s, fontSize: selectedTextBlock.font_size ?? 12 }));
    setMoveDraft({
  ...
  useEffect(() => { void fetchExistingDocs(); }, []);

  // ─── Operations ───────────────────────────────────────────────────────────────
  const queueOperation = (op: EditOperation) => dispatch({ type: "add_operation", payload: op });
  ...
  const applyQueuedOperations = async (ops: EditOperation[]) => {
    ...
    setPageOrderDraft(result.manifest.pages.map((p) => p.page_number).join(", "));
    dispatch({
      type: "apply_complete",
      payload: {
  ...
  const handleApply = () => {
    ...
    startTransition(() => {
      void (async () => {
        try { await applyQueuedOperations(state.operations); }
        catch (error) { dispatch({ type: "set_error", payload: error instanceof Error ? error.message : "Failed to apply changes." }); }
      })();
    });
  };
  ...
  const handleExport = () => {
    ...
    startTransition(() => {
      void (async () => {
        try {
          if (state.operations.length) await applyQueuedOperations(state.operations);
  ...
  const handleReset = async () => {
    ...
    dispatch({ type: "reset" });
    if (documentId) {
      try { await deleteDocument(documentId); await fetchExistingDocs(); } catch { /* best-effort */ }
    }
  };
  ...
  const layoutClass = useMemo(() => {
    if (isLeftPanelOpen && isRightPanelOpen)   return "lg:grid-cols-[280px_minmax(0,1fr)_360px]";
    if (!isLeftPanelOpen && isRightPanelOpen)  return "lg:grid-cols-[64px_minmax(0,1fr)_360px]";
    if (isLeftPanelOpen && !isRightPanelOpen)  return "lg:grid-cols-[280px_minmax(0,1fr)_64px]";
    return "lg:grid-cols-[64px_minmax(0,1fr)_64px]";
  }, [isLeftPanelOpen, isRightPanelOpen]);

  return (
    <div className={`grid gap-4 p-4 transition-all duration-300 h-full min-h-0 ${layoutClass}`}>
      <DocumentPanel
        state={state}
        dispatch={dispatch}
        isPending={isPending}
      ...
      <WorkspaceSection
        ...
        isPending={isPending}
        zoom={zoom}
        onZoomIn={() => setZoom((v) => Math.min(2.5, Number((v + 0.1).toFixed(2))))}
        onZoomOut={() => setZoom((v) => Math.max(0.5, Number((v - 0.1).toFixed(2))))}
        onApply={handleApply}
