  ...
  return (
    <div className="flex flex-col items-center justify-start h-full pt-2">
      ...
      <div className="mt-8 flex-1" style={{ writingMode: "vertical-rl", textOrientation: "mixed" }}>
        <span className="text-xs font-semibold uppercase tracking-widest text-slate-500 whitespace-nowrap">Document</span>
      </div>
    </div>
  );
}

function PageThumbnail({ page, selected, onSelect }: { page: PageManifest; selected: boolean; onSelect: () => void }) {
  return (
    <button
    ...
    >
      ...
      <div className="px-3 py-2 text-xs text-slate-500">
        Page {page.page_number} — {page.text_blocks.length} text · {page.image_blocks.length} image blocks
      </div>
    </button>
  );
}

export function DocumentPanel({ state, dispatch, isPending, isOpen, existingDocs, onToggle, onUpload, onReset, onLoadDoc, onDeleteDoc }: Props) {
  return (
    <aside className="glass-panel rounded-[24px] p-4 shadow-panel flex flex-col h-full min-h-0">
      {!isOpen ? (
        <CollapsedSliver onToggle={onToggle} />
      ) : (
        <div className="flex flex-col h-full min-h-0 opacity-100 transition-opacity duration-300">
          {/* Header */}
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              {state.documentId ? (
                <button className="rounded-full border border-slate-300 px-3 py-1.5 text-xs text-slate-700" onClick={onReset} type="button">
                  Reset
                </button>
              ) : null}
              <button
                className="rounded-full bg-slate-100 p-1.5 text-slate-700 hover:bg-slate-200"
                onClick={onToggle}
          ...
          <label className="mt-4 shrink-0 flex cursor-pointer flex-col items-center justify-center rounded-3xl border border-dashed border-slate-300 bg-white/80 px-4 py-8 text-center">
            ...
            <span className="mt-2 text-xs leading-5 text-slate-500">Upload, queue edits, apply, and export only when the preview is right.</span>
            <input accept="application/pdf" className="hidden" onChange={onUpload} type="file" />
          </label>

          {/* Existing docs */}
          {existingDocs.length > 0 && (
            <div className="mt-4 shrink-0 flex flex-col max-h-44">
              <p className="text-xs uppercase tracking-[0.28em] text-teal-700">Existing Documents</p>
              <div className="mt-2 space-y-2 overflow-y-auto pr-1">
                {existingDocs.map((doc, idx) => (
                  <div key={idx} className="flex items-center justify-between rounded-xl bg-white/60 px-3 py-2 text-xs border border-slate-200">
                    <button
                      type="button"
          ...
          {state.manifest ? (
            <div className="mt-4 flex flex-col flex-1 min-h-0">
              <div className="shrink-0 rounded-2xl bg-slate-900 px-4 py-3 text-sm text-white mb-3">
                ...
                <p className="mt-1 text-slate-300">
                  {state.manifest.page_count} pages &mdash; {state.manifest.detected_fonts.length} fonts detected
                </p>
              </div>
              <div className="space-y-3 overflow-y-auto flex-1 min-h-0 pr-1">
                {state.manifest.pages.map((page) => (
                  <PageThumbnail
                    ...
                    onSelect={() => dispatch({ type: "select_page", payload: page.page_number })}
                  />
                ))}
              </div>
            </div>
            ...
            <div className="mt-4 rounded-2xl bg-slate-100 px-4 py-4 text-sm text-slate-600">
              The current working PDF and its pages will appear here after upload.
            </div>
          )}
        </div>
      )}
